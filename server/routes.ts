...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ─── Challenge Will Endpoints ─────────────────────────────────────────────

  app.post('/api/wills/challenge', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const { what, because, visibility, type, startDate, endDate, isIndefinite, title } = req.body;
      if (!what) return res.status(400).json({ message: 'Goal text is required' });
      ...
      const [will] = await db.insert(wills).values({
        ...
        isIndefinite: isIndefinite ?? !endDate,
        checkInType: 'daily',
        activeDays: 'every_day',
      }).returning();

      await db.insert(willCommitments).values({ willId: will.id, userId, what, because: because || null });
      res.json({ id: will.id });
    } catch (err) {
  ...
  async function computeChallengeLeaderboard(
    ...
    const childWills = await db
      .select()
      .from(wills)
      .where(and(eq(wills.parentWillId, challengeId), inArray(wills.status, ACTIVE_PARTICIPANT_STATUSES)));

    const allEntries = [
      { willId: challengeId, userId: will.createdBy, isCreator: true },
      ...childWills.map((cw: any) => ({ willId: cw.id, userId: cw.createdBy, isCreator: false })),
    ];

    const competitors = await Promise.all(
      allEntries.map(async ({ willId, userId: compUserId, isCreator }) => {
        const [u] = await db.select({ firstName: users.firstName }).from(users).where(eq(users.id, compUserId));
        const progress = await storage.getWillCheckInProgress(willId, compUserId);
        const effectiveTotal = challengeTotalDays ?? daysIn;
        const mySuccessRate = daysIn > 0 ? Math.round(progress.yesCount / Math.max(daysIn, 1) * 100) : 0;
        const score = Math.min(100, Math.round((mySuccessRate * 0.6) + (progress.yesCount / Math.max(effectiveTotal, 1) * 100 * 0.4)));
        return {
          userId: compUserId,
          firstName: u?.firstName || 'Anonymous',
          isCreator,
          isYou: compUserId === requestingUserId,
          successRate: mySuccessRate,
          totalDaysCompleted: progress.yesCount,
          totalDays: daysIn,
          score,
          willId,
        };
    ...
    competitors.sort((a, b) => b.score - a.score || a.firstName.localeCompare(b.firstName));
    return competitors;
  }

  app.get('/api/wills/challenge/:id', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const challengeId = parseInt(req.params.id);
      if (isNaN(challengeId)) return res.status(400).json({ message: 'Invalid challenge id' });

      const will = await storage.getWillById(challengeId);
      if (!will || will.kind !== 'challenge') return res.status(404).json({ message: 'Challenge not found' });

      const isOwner = will.createdBy === userId;
      const myChildWill = isOwner ? null : await storage.getUserJoinedWill(userId, challengeId);

      if (will.visibility === 'private' && !isOwner && !myChildWill) {
        return res.json({ id: challengeId, visibility: 'private', blocked: true });
      }

      const [commitment] = await db.select({ what: willCommitments.what }).from(willCommitments).where(eq(willCommitments.willId, challengeId)).limit(1);
      const [creator] = await db.select({ firstName: users.firstName }).from(users).where(eq(users.id, will.createdBy));

      const now = Date.now();
      const startMs = will.startDate ? new Date(will.startDate as string).getTime() : now;
      const daysIn = Math.max(1, Math.floor((now - startMs) / 86400000) + 1);
      const challengeTotalDays = will.endDate && !will.isIndefinite
        ? Math.ceil((new Date(will.endDate as string).getTime() - startMs) / 86400000)
        : null;
      const daysLeft = challengeTotalDays ? Math.max(0, challengeTotalDays - daysIn) : null;

      const renderState: 'owner' | 'participant' | 'viewer' = isOwner ? 'owner' : myChildWill ? 'participant' : 'viewer';
      const myWillId = isOwner ? challengeId : (myChildWill?.id ?? null);

      const today = new Date().toISOString().split('T')[0];
      let checkedInToday = false;
      if (myWillId) {
        const todayCheckIns = await storage.getWillCheckIns(myWillId, userId);
        checkedInToday = todayCheckIns.some((ci: any) => ci.date === today);
      }

      const competitors = await computeChallengeLeaderboard(challengeId, userId, will, daysIn, challengeTotalDays);

      res.json({
        id: will.id,
        title: will.title ?? null,
        what: commitment?.what || 'Untitled challenge',
        visibility: will.visibility,
        type: will.type,
  ...
  app.post('/api/wills/challenge/:id/join', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const challengeId = parseInt(req.params.id);
      if (isNaN(challengeId)) return res.status(400).json({ message: 'Invalid challenge id' });
      ...
      const will = await storage.getWillById(challengeId);
      if (!will || will.kind !== 'challenge') return res.status(404).json({ message: 'Challenge not found' });
      if (will.visibility !== 'open') return res.status(403).json({ message: 'This challenge is private' });
      if (will.createdBy === userId) return res.status(400).json({ message: 'You are already the creator' });

      const existing = await storage.getUserJoinedWill(userId, challengeId);
      if (existing) return res.status(409).json({ message: 'Already joined' });

      const [childWill] = await db.insert(wills).values({
        createdBy: userId,
        parentWillId: challengeId,
        kind: 'challenge',
        visibility: 'private',
        mode: 'personal',
        status: (will.status as any) || 'active',
        type: will.type,
        startDate: will.startDate,
        endDate: will.endDate,
        isIndefinite: will.isIndefinite,
      ...
      const [parentCommitment] = await db.select().from(willCommitments).where(eq(willCommitments.willId, challengeId)).limit(1);
      if (parentCommitment) {
        await db.insert(willCommitments).values({ willId: childWill.id, userId, what: (parentCommitment as any).what, because: null });
      }

      res.json({ id: childWill.id });
    } catch (err) {
  ...
  app.get('/api/wills/challenge/:id/all-competitors', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const challengeId = parseInt(req.params.id);
      if (isNaN(challengeId)) return res.status(400).json({ message: 'Invalid challenge id' });
      ...
      const competitors = await computeChallengeLeaderboard(challengeId, userId, will, daysIn, challengeTotalDays);
      res.json({ competitors, competitorCount: competitors.length });
    } catch (err) {
