...
- **Today**: Ephemeral daily expression feature — a lightweight journal/intention space. No tracking, no streaks, no social layer. Auto-saves text as user types (debounced 1.5s, flushed on unmount). Resets each day at midnight. Data stored in `today_entries` table keyed by userId + date (YYYY-MM-DD) with unique constraint. Accessible from Home screen via "Today" card. Route: `/today`.
