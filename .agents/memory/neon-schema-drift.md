...
**Drift also affects DATA migrations, not just columns.** `server/migrations/001-will-kind-visibility.ts` renames `wills.visibility` value `'public'` → `'open'` (canonical "public" flag). This never ran on prod, so prod still holds active public solo wills with the legacy `visibility='public'`. Any query that hard-codes `visibility='open'` silently drops them. **Lesson:** when reading the "public" flag, accept BOTH values (`visibility IN ('open','public')`) rather than trusting that the rename migration ran in prod.
