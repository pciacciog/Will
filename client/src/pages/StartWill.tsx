...
import { ArrowRight, Calendar, Clock, Target, HelpCircle, CheckCircle, Heart, Video, Users, ClipboardList, MessageCircle, CalendarDays } from "lucide-react";
...
export default function StartWill({ isSoloMode = false, circleId }: StartWillProps) {
  ...
  // Visibility: derived from URL param — 'public' for Public Will creation, 'open' for everything else.
  const visibility = (() => {
    const params = new URLSearchParams(window.location.search);
    return params.get('visibility') === 'public' ? 'public' as const : 'open' as const;
  })();
  ...
  const handleConfirmSubmit = () => {
    ...
    if (ENABLE_END_ROOM && !isSoloMode) {
      ...
      createWillMutation.mutate({
        ...
        activeDays: notificationsData?.trackedDays ? 'custom' : 'every_day',
        customDays: notificationsData?.trackedDays ? JSON.stringify(notificationsData.trackedDays) : undefined,
    ...
    <div className="w-full max-w-screen-sm mx-auto overflow-x-hidden bg-gradient-to-br from-gray-50 via-white to-gray-50/50 min-h-screen">
      ...
      <MobileLayout>
        ...
        <div className={`sticky top-0 z-10 bg-white border-b border-gray-100 pt-[calc(env(safe-area-inset-top)+1rem)] ${currentStep === 5 ? "pb-2 mb-2" : "pb-4 mb-6"}`}>
          ...
          {/* Current Step Title — hidden on review step and notifications step (component renders its own heading) */}
          ...
          <div className="text-center mt-3">
            <h1 className="text-xl font-semibold text-gray-900">
              ...
              {!showTypeSelection && currentStep === 4 && "Notifications Setup"}
            </h1>
