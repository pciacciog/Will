...
export default function DirectMessages({
  ...
  const { data: rawMessages, isLoading } = useQuery<DMMessage[]>({
    queryKey: ["/api/dm", otherUserId],
    queryFn: async () => {
      const res = await apiRequest(`/api/dm/${otherUserId}`);
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      if (newKeyboardHeight > 0) setTimeout(() => scrollToBottom("smooth"), 100);
    };
    viewport.addEventListener("resize", onResize);
    viewport.addEventListener("scroll", onResize);
    return () => { viewport.removeEventListener("resize", onResize); viewport.removeEventListener("scroll", onResize); };
  }, [scrollToBottom]);
  ...
  return (
    <div className="flex flex-col h-full relative">
      <div
        ...
        style={{ minHeight: 0, paddingBottom: inputBarHeight + bottomOffset + (keyboardHeight > 0 ? 8 : safeAreaBottom + 8) }}
      >
        {messages.length === 0 ? (
                ...
                {showDaySeparator && (
                  <div className="flex items-center justify-center my-3">
                    <div className="bg-gray-200 rounded-full px-3 py-0.5">
                      <span className="text-[11px] font-medium text-gray-500">{formatDayLabel(msg.createdAt)}</span>
                    </div>
                  </div>
                )}
                <div className={`flex ${isOwn ? "justify-end" : "justify-start"} mb-1.5`} data-testid={`dm-message-${msg.id}`}>
                  <div className={`max-w-[80%] rounded-2xl px-3 py-1.5 ${isOwn ? "bg-gradient-to-br from-emerald-500 to-teal-500 text-white rounded-br-md" : "bg-gray-100 text-gray-900 rounded-bl-md"}`}>
                    <p className="text-sm whitespace-pre-wrap break-words leading-snug">{msg.text}</p>
                    <p className={`text-[10px] leading-none mt-0.5 text-right ${isOwn ? "text-white/60" : "text-gray-400"}`}>
      ...
      <div
        ...
        style={{ bottom: bottomOffset, paddingBottom: keyboardHeight > 0 ? 8 : "calc(env(safe-area-inset-bottom, 0px) + 8px)" }}
      >
        {text.length > 400 && (
          <p className="text-[10px] text-gray-400 text-right mb-1">{text.length}/500</p>
        )}
